...
export default function CheckoutCart() {
	...
	const [optionData,setOptionData] = useState(['0','1','2','3','4','5']);
...
<div style={{display: 'block'}} id="w3lssbmincart" className="check-out-cart-wrap">
		 ...
		 <div className="sbmincart-footer">       
			<div className="sbmincart-subtotal radio-wrap">  
				<input name="pay_type" type="radio"/><span>Kapıda Kredi Kartı ile ödeme seçeneği</span>
				<input type="radio" name="pay_type" /><span>Kapıda Nakit Ödeme Seçeneği</span>
			  </div> 
		   <div className="sbmincart-subtotal">  
		   Toplam Tutar:{getTotalAmount()}
		   <div>
		   		<button onClick={()=>checkout_order()}>Siparişi Onaylıyorum</button>   																				
			</div>    
		 </div>       
				   </div>  
